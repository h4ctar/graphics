// @ts-check
/**
 * @typedef { import("./math").Point2Data } Point2Data
...
/**
 * @param {{ point: Point3Data, texel: Texel }} p1
 * @param {{ point: Point3Data, texel: Texel }} p2
 * @param {{ point: Point3Data, texel: Texel }} p3
 * @param {ImageData} texture
 */
...
const clipLine = (a, b, plane) => {
    ...
    const da = a.point.dot(plane.normal) - plane.distance;
    const db = b.point.dot(plane.normal) - plane.distance;
    ...
    if (Math.sign(da) !== Math.sign(db)) {
        ...
        clippedPoints.push({
            point: b.point.sub(a.point).mul(s).add(a.point),
            texel: {
